...
public class IPAddressUtil { 
 ...
 public static String getAddresses(String content, String encodingString)
  ...
  String returnStr = "";
	  try
	  {
		  returnStr = getResult(urlStr, content, encodingString);
	  }catch(Exception e)
	  {
	  }
  if (returnStr != null) {
...
//   region = decodeUnicode(region);// 省份
    ...
    for(int i=0;i<temp.length;i++){
	    switch(i){ 
		    case 1:
//		    country =(temp[i].split(":"))[2].replaceAll("\"", ""); country =
//		    decodeUnicode(country);//国家 
		    break; 
		    case 3:
//		    area =(temp[i].split(":"))[1].replaceAll("\"", ""); area =
//		    decodeUnicode(area);//地区 
		    break; 
		    case 5:
		    region =(temp[i].split(":"))[1].replaceAll("\"", ""); region =
		    decodeUnicode(region);//省份
		    break; 
		    case 7:
		    city =(temp[i].split(":"))[1].replaceAll("\"", ""); city =
		    decodeUnicode(city);//市区 
		    break; 
		    case 9:
		    county =(temp[i].split(":"))[1].replaceAll("\"", ""); county =
		    decodeUnicode(county);//地区 
		    break; 
		    case 11:
//		    isp =(temp[i].split(":"))[1].replaceAll("\"", ""); isp =
